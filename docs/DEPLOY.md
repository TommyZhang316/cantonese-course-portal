...
1. 保留Email/Password登入及電郵驗證，新密碼下限6字元。
...
6. 已有電郵教職員繼續使用原有登入。新增電郵教職員由專案負責人安排建立及驗證，再到Portal設定權限。管理員亦可先建立拼音帳戶並設定老師或管理員角色；當事人完成首次改密碼後，才可使用該角色的材料及功能。

短姓名的初始密碼仍按帳戶名稱輸入，例如`LIWU`；6字元限制適用於使用者自行設定的新密碼。
