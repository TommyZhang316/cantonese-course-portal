...
export function StudyView({ account, lessons, resources }: { account: Account; lessons: Lesson[]; resources: Resource[] }) {
  ...
  return <div className="study-layout">
    <aside className="course-sidebar" aria-label="課程導覽"><div className="sidebar-course"><span className="eyebrow">由聽懂，到開口講</span><strong>我們的粵語課堂</strong><span>{lessons.length} 堂 · 課程材料</span></div><nav className="course-navigation" aria-label="材料範圍"><button className={`sidebar-link ${selected === 'overview' ? 'is-selected' : ''}`} onClick={() => choose('overview')} aria-current={selected === 'overview' ? 'page' : undefined}><BookOpen size={21} aria-hidden="true"/>課程總覽</button><button className={`sidebar-link ${selected === 'all' ? 'is-selected' : ''}`} onClick={() => choose('all')} aria-current={selected === 'all' ? 'page' : undefined}><Folders size={21} aria-hidden="true"/>全部材料<span className="nav-count">{available.length}</span></button><div className="sidebar-label">每堂學習</div>{lessons.map((item, index) => <button key={item.id} className={`lesson-nav ${selected === item.id ? 'is-selected' : ''}`} aria-current={selected === item.id ? 'page' : undefined} onClick={() => choose(item.id)}><span className="lesson-nav-number">{String(index + 1).padStart(2, '0')}</span><span><strong>{item.title}</strong><small>{formatDate(item.starts_at, false)}</small></span></button>)}<button className={`sidebar-link shared-nav ${selected === 'shared' ? 'is-selected' : ''}`} onClick={() => choose('shared')} aria-current={selected === 'shared' ? 'page' : undefined}><Folders size={21} aria-hidden="true"/>共用資料</button></nav><div className="sidebar-note"><Clock size={18} aria-hidden="true"/><span>所有日期及發布時間<br/>均為香港時間（UTC+8）。</span></div></aside>
    <main id="main-content" className="study-main" tabIndex={-1}>
      ...
      {selected === 'overview' ? <>
        ...
        <section aria-labelledby="journey-title" className="journey-section"><div className="section-heading"><div><p className="eyebrow">一步一步，學識開口</p><h2 id="journey-title">你的學習旅程</h2></div><span className="section-count">共 {lessons.length} 堂</span></div><div className="lesson-grid">{lessons.map((item, index) => { const count = available.filter(resource => resource.lesson_id === item.id).length; const past = item.ends_at && new Date(item.ends_at).getTime() < portalNow(); return <button className="lesson-card" key={item.id} onClick={() => choose(item.id)}><div className="lesson-card-top"><span className="lesson-card-number">{String(index + 1).padStart(2, '0')}</span><span className="lesson-card-date">{formatDate(item.starts_at, false)}</span></div><h3>{item.title}</h3><p>{item.summary}</p><div className="lesson-card-bottom"><span>{past && <Check size={15} aria-hidden="true"/>}{count ? `${count} 份${teacher ? '課程' : '已開放'}材料` : '材料尚未開放'}</span><ArrowRight size={20} aria-hidden="true"/></div></button>; })}</div></section>
