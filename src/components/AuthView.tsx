...
export function AuthView({ onComplete, recovery = false, requiredPasswordChange }: { onComplete: () => void; recovery?: boolean; requiredPasswordChange?: Account }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (busy) return;
    if (mode === 'recovery' && password.length < 6) { setError('請使用至少 6 個字元的新密碼。'); return; }
  ...
  return <main id="main-content" className="auth-layout" tabIndex={-1}>
    ...
    <section className="auth-panel" aria-labelledby="auth-title"><div className="auth-form-inner"><div className="auth-kicker">{mode === 'login' ? '你好 nei5 hou2' : '公益粵語課堂'}</div><h2 id="auth-title">{title}</h2><p className="muted">{mode === 'login' ? '使用管理員提供的帳戶，繼續今天的學習。' : mode === 'reset' ? '教職員可用登記電郵收取重設連結。' : requiredPasswordChange ? '設定後便可進入課堂，閱讀已發布材料。' : '請選擇至少 6 個字元的新密碼。'}</p>
      ...
      <form onSubmit={submit} className="stack-form">
        ...
        {mode !== 'reset' && <Field label={mode === 'recovery' ? '新密碼' : '密碼'} required hint={mode === 'recovery' ? '至少 6 個字元，建議使用一段容易記住的長句。' : '首次登入請使用管理員提供的初始密碼，與帳戶名稱相同。'}><Input type="password" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} contentBefore={<LockKey size={19} aria-hidden="true"/>} value={password} onChange={(_, data) => setPassword(data.value)} minLength={mode === 'login' ? undefined : 6} maxLength={256} required disabled={busy}/></Field>}
        {mode === 'recovery' && <Field label="再次輸入密碼" required><Input type="password" autoComplete="new-password" value={confirmation} onChange={(_, data) => setConfirmation(data.value)} minLength={6} maxLength={256} required disabled={busy}/></Field>}
