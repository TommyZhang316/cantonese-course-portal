...
export default function App() {
  ...
  const refresh = useCallback(async (clear = false) => {
    ...
    try {
      ...
      const nextIdentity = nextAccount ? `${nextAccount.id}:${nextAccount.role}:${nextAccount.status}` : '';
      if (nextIdentity !== identity.current) { identity.current = nextIdentity; setResources([]); setLessons([]); setPage('study'); setLoading(true); }
      setAccount(nextAccount);
      if (!nextAccount || nextAccount.status !== 'active') { setResources([]); setLessons([]); setError(''); return; }
